...
- **신규 점포 생존**은 수요 시점과 생존 시점이 같아 시간 순서가 맞는 검증입니다. 결정 10 모형(이 점검 전에 이론으로 확정)의 $\tilde R$만 구간이 0보다 크고, 경쟁만 보는 기준보다도 유의하게 높습니다. 업종별로는 중식 0.21, 일식 0.18이 가장 강합니다. 효과 크기는 작습니다.
...
| 10 | 층 2를 준포아송 회귀 + 목적형 상권수요로 변경(이론 근거로 사전 확정). 검증은 신규 점포 생존 중심 — **구현 완료** |
