...
| 층 2 상권수요 λ 5·20km 민감도 (`lambda_sensitivity.csv`) | 목적형 4업종(한식·중식·일식·서양) 순위상관 0.95~0.99, 개설 목록 겹침(Jaccard) 0.82~0.93 — 주 분석(λ=10km) 대비 매우 안정적. 신규 점포 생존 ρ: λ=5 0.064 [0.026, 0.107], λ=20 0.076 [0.040, 0.115]로 주 분석 0.075 [0.040, 0.115]와 비슷하고, 공급만 대비 구간은 두 λ 모두 0보다 큼 |
...
| 10 | 층 2를 준포아송 회귀 + 목적형 상권수요로 변경(생존 점검 전 확정, 근거는 계수 자료·결정 9). 검증은 신규 점포 생존 중심 (λ 5·20km 민감도) — **구현 완료** |
