...
def over_index(bc: pl.DataFrame, regions: pl.DataFrame) -> np.ndarray:
    ...
    j = regions.join(bc_age6, on=KEY, how="left").join(bc_total, on=KEY, how="left") \
        .join(pop_age6, on=KEY, how="left").join(pop_total, on=KEY, how="left")
...
def selftest() -> None:
    ...
    I, B = 255, 8
    o, t = rng.normal(size=I), rng.normal(size=B)
    x = o[:, None] * t[None, :]
    x_tilde = x - x.mean(1, keepdims=True)  # 지역 안 업종 평균 중심화
    industry_effect = rng.normal(size=B)
...
def main() -> None:
    ...
    k_mean, (k_lo, k_hi) = kappa.mean(), np.quantile(kappa, [0.05, 0.95])
    r2_mean = partial_r2.mean()
    print(f"kappa 사후평균={k_mean:.4f} 90% 구간=[{k_lo:.4f}, {k_hi:.4f}] 부분 R² 평균={r2_mean:.4f}")

    R_mean = R_rel.mean(0)  # (255, 8) 사후평균
    R_adj = R_mean - k_mean * x_tilde  # 조정: 고령 구성 효과 제거

    depop = regions.join(pl.read_csv(EXT / "subsidy_tier_sgg_2026.csv").select(*KEY, "SUBSIDY_10K_KRW"), on=KEY, how="left")
    ...
    for b, name in enumerate(industries):
        ...
        after[np.argsort(-R_adj[:, b])[:n_top]] = True
        jaccard = (before & after).sum() / (before | after).sum()
        rows.append((name, float(t[b]), jaccard, int((before & depop).sum()), int((after & depop).sum())))

    table = pl.DataFrame(rows, schema=["b", "t_b", "jaccard_top25_after_adjust", "n_depop_top25_before", "n_depop_top25_after"], orient="row")
    table.write_csv(out / "share_bias_check.csv")
    print(table)

    signal = (k_lo > 0 or k_hi < 0) and r2_mean >= 0.05
    print("점유율 구성 편향 신호 있음" if signal else "점유율 구성 편향 신호 없음")
